...
namespace ConsoleApp1
{
	internal static class Program
	{
		...
		public static async Task GenerateImage(Template template, Product product)
		{
			...
			var orderdTemplateElements = templateElements.OrderBy(e => e.Z_Index);
		...
		private static void LayerText(TextElement element, Product product, Image canvas)
		{
			...
			var dynamicText = BuildDynamicText(productJObject, element.Text);
			...
			textBackground.Mutate(a => a.DrawText(textGraphicsOptions, dynamicText, font, Color.FromRgb(element.CR, element.CG, element.CB), new PointF(0, 0)));
		...
		// TODO:  this will be done only once, for all the custom fonts in the template.
